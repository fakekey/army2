...
import com.army2.server.ClientEntry;
import com.army2.server.GameString;
import com.army2.server.ItemData;
import com.army2.server.MapData;
import com.army2.server.Room;
    ...
    public void startMessage(ClientEntry cl) throws IOException {
        ...
        for (byte i = 0; i < this.players.length; i++) {
            ...
            for (byte j = 0; j < 8; j++) {
                ...
                if ((j >= 4 && itemSelect > 0 && pl.getItemNum(12 + j - 4) <= 0) || itemMap[i][itemSelect] > ItemData.nItemDcMang[itemSelect]
    ...
    public void setMapMessage(User us, Message ms) throws IOException {
        ...
        if (this.parent.slMap.size() > 0) {
            ...
            if (this.parent.maxMap == this.parent.minMap) {
                ...
                ds.writeUTF(String.format(GameString.selectMapError1_2(), MapData.entrys.get(this.parent.minMap).name,
                        MapData.entrys.get(this.parent.maxMap).name));
            } else {
                ds.writeUTF(GameString.selectMapError1_3());
            }
            ds.flush();
