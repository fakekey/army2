...
import com.army2.fight.BulletManager.AddBoss;
import com.army2.fight.BulletManager.BomHenGio;
import com.army2.fight.BulletManager.Bullets;
import com.army2.fight.BulletManager.VoiRong;
import com.army2.fight.boss.Balloon;
import com.army2.fight.boss.Balloon_FanBack;
import com.army2.fight.boss.Balloon_Gun;
import com.army2.fight.boss.Balloon_GunBig;
import com.army2.fight.boss.BigBoom;
import com.army2.fight.boss.Ghost;
import com.army2.fight.boss.Ghost2;
import com.army2.fight.boss.Robot;
import com.army2.fight.boss.SpiderMachine;
import com.army2.fight.boss.SpiderPoisonous;
import com.army2.fight.boss.Trex;
import com.army2.fight.boss.UFO;
import com.army2.fight.boss.UFOFire;
import com.army2.fight.bullet.ItemBomHenGio;
import com.army2.network.Message;
import com.army2.server.GameString;
import com.army2.server.ItemData;
import com.army2.server.ServerManager;
import com.army2.server.SpecialItemData;
import com.army2.server.Until;
import com.army2.server.User;
...
public class FightManager {
    ...
    private void nextBoss() throws IOException {
        ...
        if (this.wait.map == 32) {
            ...
            for (byte i = 0; i < numBoss; i++) {
                players[allCount] = new SpiderMachine(this, (byte) 13, "Spider Robot", (byte) allCount, 4785 + (getLevelTeam() * 15),
                        (short) tempX[i], (short) tempY[i]);
                allCount++;
            }
        }
        // map thanh pho may
        ...
        if (this.wait.map == 36) {
            ...
            allCount++;
            players[allCount] = new Balloon_Gun(this, (byte) 18, "Balloon Gun", (byte) allCount, 2000 + (this.getLevelTeam() * 10), (short) (X + 51),
                    (short) (Y + 19));
            allCount++;
            players[allCount] = new Balloon_GunBig(this, (byte) 19, "Balloon Gun Big", (byte) allCount, 2500 + (this.getLevelTeam() * 10),
                    (short) (X - 5), (short) (Y + 30));
            allCount++;
            players[allCount] = new Balloon_FanBack(this, (byte) 20, "Fan Back", (byte) allCount, 1000 + (this.getLevelTeam() * 10), (short) (X - 67),
                    (short) (Y - 6));
            allCount++;
        }
        ...
        if (this.wait.map == 37) {
            ...
            for (byte i = 0; i < numBoss; i++) {
                ...
                short Y = (short) 250;
                players[allCount] =
                        new SpiderPoisonous(this, (byte) 22, "Spider Poisonous", (byte) allCount, 3800 + (this.getLevelTeam() * 10), X, Y);
                allCount++;
            }
        }
    ...
    /*
     * private void huyCantMove(int index) throws IOException { Message ms = new Message(107);
     * DataOutputStream ds = ms.writer(); ds.writeByte(1); ds.writeByte(index); ds.flush();
     * this.sendToTeam(ms); }
     */
        ...
        if (this.ltap) {
            ...
            if (this.playerTurn == -1) {
                while (true) {
                    ...
                    if (this.players[next] != null && this.players[next].idNV != 18 && this.players[next].idNV != 19 && this.players[next].idNV != 20
                            && this.players[next].idNV != 21 && this.players[next].idNV != 23 && this.players[next].idNV != 24) {
                ...
                if (this.type == 5) {
                    if (this.isBossTurn) {
                        ...
                        while (turn != this.playerTurn) {
                            ...
                            if (this.players[turn] != null && !this.players[turn].isDie && this.players[turn].idNV != 18
                                    && this.players[turn].idNV != 19 && this.players[turn].idNV != 20 && this.players[turn].idNV != 21
                                    && this.players[turn].idNV != 23 && this.players[turn].idNV != 24) {
                    ...
                    while (turn != this.playerTurn) {
                        ...
                        if (this.players[turn] != null && !this.players[turn].isDie && this.players[turn].idNV != 18 && this.players[turn].idNV != 19
                                && this.players[turn].idNV != 20 && this.players[turn].idNV != 21 && this.players[turn].idNV != 23
                                && this.players[turn].idNV != 24) {
                            this.playerTurn = (byte) turn;
                            break;
                        }
                        turn++;
    ...
    private void fightComplete(byte checkWin) throws IOException {
        ...
        if (!this.wait.isLH && this.type == 5 && checkWin == 1) {
            for (byte i = 0; i < ServerManager.maxPlayers; i++) {
                ...
                String strItem = numItem + "x " + ItemData.entrys.get(idItem).name + " " + numSpItem + "x "
                        + SpecialItemData.getSpecialItemById(idSpItem).name + " " + numSpItemsk + "x Bông tuyết";
        ...
        if (wait.isLH) {
            for (byte i = 0; i < ServerManager.maxPlayers; i++) {
                ...
                if (LHfinish) {
                    ...
                    pl.us.sendMSSToUser(null, String.format(GameString.LHFinish(),
                            Until.getStringNumber(arXP[arXP.length - 1] * xXu) + " XP +" + Until.getStringNumber(arXu[arXu.length - 1]) + " Xu"));
    ...
    protected void startGame(int nTeamPointBlue, int nTeamPointRed) throws IOException {
        ...
        if (this.ltap) {
            ...
            this.players[0] =
                    new Player(this, (byte) 0, ServerManager.Xltap[0], ServerManager.Yltap[0], Player.getLuyenTapItem(), (byte) 0, this.userLt);
            this.players[1] =
                    new Player(this, (byte) 1, ServerManager.Xltap[1], ServerManager.Yltap[1], Player.getLuyenTapItem(), (byte) 0, this.userLt);
            for (int i = 2; i < 8; i++) {
                this.players[i] = null;
            }
        } else {
    ...
    public void newShoot(int index, byte bullId, short arg, byte force, byte force2, byte nshoot, boolean ltap) throws IOException {
        ServerManager.log("New shoot index=" + index + " bullId: " + bullId + " arg: " + arg + " force: " + force + " force2: " + force2 + " nshoot: "
                + nshoot);
    ...
    public void shootMessage(User us, Message ms) throws IOException {
        ...
        newShoot(index, bullId, (arg > 360 ? 360 : (arg < -360 ? -360 : arg)), (force > 30 ? 30 : (force < 0 ? 0 : force)),
                (force2 > 30 ? 30 : (force2 < 0 ? 0 : force2)), nshoot, ltap);
    }
    ...
    public void useItemMessage(User us, Message ms) throws IOException {
        ...
        if (idItem == 0 || idItem == 2 || idItem == 3 || idItem == 4 || idItem == 5 || idItem == 10 || idItem == 32 || idItem == 33 || idItem == 34
                || idItem == 35 || idItem == 100) {
            pl.itemUsed = -1;
        }
    }
    ...
    public short[] getForceArgXY(int idGun, BulletManager bull, boolean isXuyenMap, short X, short Y, short toX, short toY, short Mx, short My,
            int arg, int force, int msg, int g100) {
    ...
    private byte getCollisionPoint(BulletManager bull, short X1, short Y1, short X2, short Y2, short toX, short toY, short Mx, short My,
            boolean isXuyenMap) {
        int Dx = X2 - X1;
        int Dy = Y2 - Y1;
        byte x_unit = 0;
