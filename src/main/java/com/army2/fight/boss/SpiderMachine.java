    ...
    public void turnAction() {
        try {
            ...
            if (Math.abs(X - pl.X) <= 40 && Math.abs(Y - pl.Y) <= 30) {
                ...
                this.fightMNG.newShoot(this.index, (byte) 8, (short) Until.getArgXY(X, Y, pl.X, pl.Y), (byte) 30, (byte) 0, (byte) 1, false);
            ...
            short[] FA = null;
            switch (Until.nextInt(3)) {
                // tơ nhện
                case 0:
                    this.itemUsed = 9;
                    FA = fightMNG.getForceArgXY(idNV, this.fightMNG.bullMNG, false, X, Y, pl.X, (short) (pl.Y - (pl.height / 2)),
                            (short) (pl.width / 2), pl.height, 50, 5, 70, 70);
                    if (FA == null) {
                ...
                case 1:
                    this.itemUsed = 16;
                    FA = fightMNG.getForceArgXY(idNV, this.fightMNG.bullMNG, false, X, Y, pl.X, (short) (pl.Y - (pl.height / 2)),
                            (short) (pl.width / 2), pl.height, 50, 5, 10, 50);
                    if (FA == null) {
                        if (!fightMNG.checkWin())
                            fightMNG.nextTurn();
                        return;
                ...
                // rôcet
                case 2:
                    FA = fightMNG.getForceArgXY(idNV, this.fightMNG.bullMNG, true, X, Y, pl.X, (short) (pl.Y - (pl.height / 2)),
                            (short) (pl.width / 2), pl.height, 50, 5, 30, 50);
                    if (FA == null) {
                        if (!fightMNG.checkWin())
                            fightMNG.nextTurn();
                        return;
