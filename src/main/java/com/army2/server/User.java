...
import com.army2.fight.FightWait;
import com.army2.network.Message;
import com.army2.server.FomularData.FomularDataEntry;
import com.army2.server.FomularData.FomularEntry;
import com.army2.server.ItemData.ItemEntry;
import com.army2.server.MissionData.MissDataEntry;
import com.army2.server.MissionData.MissionEntry;
import com.army2.server.NVData.EquipmentEntry;
import com.army2.server.NVData.NVEntry;
import com.army2.server.SpecialItemData.SpecialItemEntry;
...
public class User {
    ...
    private final short[][] GIFT_DATA = {{54, 52, 53, 55, 56}, {51, 49, 50, 53, 52}, {51, 49, 50, 53, 52}, {52, 50, 51, 54, 53}, {51, 49, 50, 53, 52},
            {50, 48, 49, 52, 51}, {51, 49, 50, 53, 52}, {52, 54, 53, 50, 51}, {51, 53, 52, 49, 50}, {51, 53, 52, 49, 50}};
    ...
    public int[] getAbility() {
        ...
        ability[1] = (NVData.entrys.get(nv).sat_thuong * (100 + (envAdd[1] + this.pointAdd[nv][1]) / 3 + percenAdd[1]) / 100) * 100
                / NVData.entrys.get(nv).sat_thuong;
    ...
    protected static User login(ClientEntry cl, String user, String pass) {
        try {
            ...
            ResultSet red = SQLManager.getStatement()
                    .executeQuery("SELECT * FROM `user` WHERE (`user`=\"" + user + "\" AND `password`=\"" + pass + "\") LIMIT 1;");
            if (red != null && red.first()) {
                ...
                if (!red.first()) {
                    SQLManager.getStatement().executeUpdate(
                            "INSERT INTO armymem(`id`, `xu`, `luong`, `ruongItem`, `ruongTrangBi`) VALUES (" + us.iddb + ", 1000, 0, '[]', '[]');");
                    red.close();
                    red = SQLManager.getStatement().executeQuery("SELECT * FROM `armymem` WHERE id=\"" + us.iddb + "\" LIMIT 1;");
                    red.first();
                }
                ...
                red.close();
                // Set online
                SQLManager.getStatement().executeUpdate("UPDATE `armymem` SET `online`=1,`newOnline`='" + Until.toDateString(new Date())
    ...
    public synchronized void updateRuong(ruongDoTBEntry tbUpdate, ruongDoTBEntry addTB, int removeTB, ArrayList<ruongDoItemEntry> addItem,
            ArrayList<ruongDoItemEntry> removeItem) throws IOException {
        Message ms;
        DataOutputStream ds;
        if (addTB != null) {
            int bestLocation = -1;
    ...
    protected int flushCache() {
        ...
        try {
            // Not online
            SQLManager.getStatement().executeUpdate("UPDATE `armymem` SET `online`=0,`idapp`=-1,`lastOnline`='" + Until.toDateString(new Date())
            ...
            SQLManager.getStatement()
                    .executeUpdate("UPDATE `armymem` SET `item`='" + Jarr1.toJSONString() + "' WHERE `id`=" + this.iddb + " LIMIT 1;");
            byte nvXPMax = -1;
            int xpMax = 0;
            for (int i = 0; i < this.lever.length; i++) {
                ...
                ResultSet red =
                        SQLManager.getStatement().executeQuery("SELECT `NV" + (i + 1) + "` FROM `armymem` WHERE `id`=" + this.iddb + " LIMIT 1;");
                if ((red != null) && (red.first())) {
                    ...
                    SQLManager.getStatement().executeUpdate(
                            "UPDATE `armymem` SET `NV" + (i + 1) + "`='" + nvdata.toJSONString() + "' WHERE `id`=" + this.iddb + " LIMIT 1;");
                    if (this.xp[i] > xpMax) {
                        nvXPMax = (byte) (i + 1);
                        xpMax = this.xp[i];
                    }
            ...
            SQLManager.getStatement()
                    .executeUpdate("UPDATE `armymem` SET `ruongTrangBi`='" + Jarr1.toJSONString() + "' WHERE `id`=" + this.iddb + " LIMIT 1;");
            ...
            SQLManager.getStatement()
                    .executeUpdate("UPDATE `armymem` SET `ruongItem`='" + Jarr1.toJSONString() + "' WHERE `id`=" + this.iddb + " LIMIT 1;");
            ...
            SQLManager.getStatement()
                    .executeUpdate("UPDATE `armymem` SET `mission`='" + Jarr1.toJSONString() + "' WHERE `id`=" + this.iddb + " LIMIT 1;");
            ...
            SQLManager.getStatement()
                    .executeUpdate("UPDATE `armymem` SET `missionLevel`='" + Jarr1.toJSONString() + "' WHERE `id`=" + this.iddb + " LIMIT 1;");
            ...
            // su kien
            SQLManager.getStatement().executeUpdate("UPDATE `armymem` SET `point_event`=" + this.eventScore + ", `baodanhsk` = '"
                    + Until.toDateString(this.baodanhsk) + "' WHERE `id`=" + this.iddb + " LIMIT 1;");
            ...
            SQLManager.getStatement().executeUpdate("UPDATE `armymem` SET `nvCSinh`='" + Jarr1.toJSONString() + "', `CSinh` = '" + this.csinh
                    + "' WHERE `id`=" + this.iddb + " LIMIT 1;");
            // Xu, luong, ...
            SQLManager.getStatement()
                    .executeUpdate("UPDATE `armymem` SET `friends` = '" + this.friends.toJSONString() + "', `xu`='" + this.xu + "',`luong`='"
                            + this.luong + "',`dvong`='" + this.dvong + "',`NVused`='" + (this.nv + 1) + "',`sttnhanvat`='" + nvstt + "',`x2XPTime`='"
                            + Until.toDateString(xpX2Time) + "',`x0XPTime`='" + Until.toDateString(xpX0Time) + "',`point_event` = " + this.eventScore
    ...
    protected void changePassMessage(Message ms) throws IOException {
        ...
        try {
            ResultSet red = SQLManager.getStatement()
                    .executeQuery("SELECT `user` FROM `user` WHERE `user_id`='" + this.iddb + "' AND `password`='" + oldpass + "' LIMIT 1;");
            if (red == null || !red.first()) {
                ms = new Message(45);
                ds = ms.writer();
            ...
            if (jarr != null) {
                for (int i = jarr.size() - 1; i >= 0; i--) {
                    ...
                    red = SQLManager.getStatement()
                            .executeQuery("SELECT `xu`,`NVused`,`clan`,`online` FROM `armymem` WHERE id=\"" + iddbFR + "\" LIMIT 1;");
                    if (!red.first()) {
                        red.close();
                        continue;
                    }
    ...
    protected void viewTTMessage(Message ms) throws IOException {
        ...
        try {
            ...
            red.close();
            red = SQLManager.getStatement()
                    .executeQuery("SELECT `xu`,`luong`,`NVused`,`dvong`,`top` FROM `armymem` WHERE id=\"" + ids + "\" LIMIT 1;");
            red.first();
            ds.writeInt(red.getInt("xu"));
                                ...
                                for (byte index = 0; index < 12; index++) {
                                    ...
                                    switch (Until.nextInt(5)) {
                                        case 0:
                                            id = 55;
                                            type = 2;
                                            int[] nextXu = new int[] {1000, 2000, 5000, 10000, 12000, 15000, 20000, 25000, 30000, 40000, 50000, 70000,
                                                    100000};
