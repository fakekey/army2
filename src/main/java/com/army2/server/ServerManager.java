...
import static com.army2.server.BangXHManager.bangXHString;
import static com.army2.server.BangXHManager.bangXHString1;
import static com.army2.server.User.nvEquipDefault;
...
public class ServerManager {
    ...
    protected static String[] nameRooms =
            {"Bom", "Nhện máy", "Người máy", "T-rex máy", "UFO", "Khí cầu", "Nhện độc", "Ma", "Trùm liên hoàn", "Trùm liên hoàn"};
    ...
    protected static void getMaterialIconMessage(ClientEntry cl, Message ms) throws IOException {
        ...
        ds.writeByte(idIcon);
        if (ab0 == null) {
            ab0 = Until.getFile("src/main/resources/icon.png");
        }
    ...
    protected static void init() {
        ...
        try {
            ...
            FomularData.entrys = new ArrayList<FomularDataEntry>();
            while (res.next()) {
                    ...
                    for (BangXHEntry bxhA1 : bxhA) {
                        try {
                            ...
                            red = SQLManager.getStatement()
                                    .executeQuery("SELECT `NVused`, `nvXPMax`, `clan` FROM `armymem` WHERE id=\"" + iddb + "\" LIMIT 1;");
                            red.first();
                            byte nv = 0;
    ...
    protected static short[] data(int iddb, byte nv) {
        ...
        try {
            ResultSet red =
                    SQLManager.getStatement().executeQuery("SELECT `ruongTrangBi`,NV" + nv + " FROM armymem WHERE id=\"" + iddb + "\" LIMIT 1;");
            red.first();
            JSONObject jobj = (JSONObject) JSONValue.parse(red.getString("NV" + nv));
    ...
    private static void giftCode(User us, String code) throws IOException {
        ...
        try {
            ...
            SQLManager.getStatement().executeUpdate("UPDATE `giftcode` SET `limit` = '" + limit + "', `iddblist` = '" + iddblist.toJSONString()
