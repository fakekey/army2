...
import com.army2.server.ClanManager.ClanEntry;
                ...
                try {
                    ...
                    for (int j = 0; j < bxh.size(); j++) {
                        ...
                        SQLManager.getStatement()
                                .executeUpdate("UPDATE `armymem` SET `top`='" + bXHE.index + "' WHERE `id`=" + bXHE.iddb + " LIMIT 1;");
                        ...
                        if (bXHE.index <= 3) {
                            if (ServerManager.getUser(bXHE.iddb) != null) {
                                ...
                                SQLManager.getStatement().executeUpdate("UPDATE `armymem` SET `xu`= `xu` + " + xuUp[bXHE.index - 1] + " WHERE (`id`="
                                        + bXHE.iddb + " AND `xu` < 2000000000)LIMIT 1;");
                            }
                        }
                        if (j > 99) {
                            bxh.remove(j);
