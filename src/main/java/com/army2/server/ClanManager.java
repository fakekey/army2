...
public class ClanManager {
    ...
    protected static void getMemberClan(User us, Message ms) {
        try {
            ...
            ResultSet red = SQLManager.getStatement().executeQuery(String.format(
                    "SELECT `clanmem`.*, `armymem`.*, `user`.`user` FROM `clanmem` INNER JOIN `armymem` ON `clanmem`.`user` = `armymem`.`id` INNER JOIN `user` ON `clanmem`.`user` = `user`.`user_id` WHERE `clanmem`.`clan` = %d ORDER BY `clanmem`.`rights` DESC, `clanmem`.`xp` DESC LIMIT %d, 10;",
                    Ids, (page * 10)));
            ...
            for (int i = 0; i < length; i++) {
                ...
                ds.writeUTF(
                        memClan.getName() + (memClan.getRight() == 2 ? " (Đội trưởng)" : (memClan.getRight() > 0 ? (" (Đội phó " + i + ")") : ""))); // tên
                                                                                                                                                     // nv
                ds.writeInt(1);
                ...
                if (memClan.getN_contribute() > 0) {
                    ds.writeUTF("Góp " + memClan.getContribute_text() + " "
                            + Until.getStrungTime((new Date().getTime() - Until.getDate(memClan.getContribute_time()).getTime())) + " trước");
                } else {
                    ds.writeUTF("Chưa đóng góp");
                }
                ds.writeUTF(
                        memClan.getN_contribute() > 0
                                ? (memClan.getN_contribute() + " lần: " + Until.getStringNumber(memClan.getXu()) + " xu và "
                                        + Until.getStringNumber(memClan.getLuong()) + " lượng")
                                : "");
            }
            member.clear();
            ...
            try {
                ...
                if (red.first()) {
                    ...
                    SQLManager.getStatement()
                            .executeUpdate("UPDATE `clan` SET `level` = " + level + ",`xp` = `xp` + " + xp + "  WHERE `id` = " + us.getClan() + ";");
                    SQLManager.getStatement()
                            .executeUpdate("UPDATE `armymem` SET `clanpoint` = `clanpoint` + " + xp + " WHERE `id` = " + us.getIDDB() + ";");
                }
                red.close();
    ...
    public static void updateLuong(User us, int luongUp) {
        ...
        try {
            ...
            if (luongUp > 0) {
                SQLManager.getStatement()
                        .executeUpdate("UPDATE `clanmem` SET `luong` = `luong` + " + luongUp + " WHERE `user` = " + us.getIDDB() + ";");
            }
        } catch (SQLException e) {
            e.printStackTrace();
        }
    ...
    protected static void updateItemClan(User us, int Ids, int time) {
        if (us.getClan() > 0) {
            ...
            try {
                ...
                red.close();
                SQLManager.getStatement()
                        .executeUpdate("UPDATE `clan` SET `item`='" + itemClan.toJSONString() + "' WHERE `id`=" + us.getClan() + " LIMIT 1;");
            } catch (SQLException e) {
                e.printStackTrace();
            }
        }
        ...
        if (us.getClan() > 0) {
            try {
                ...
                if (type == 0) {
                    ...
                    us.updateXu(-soluong);
                    SQLManager.getStatement()
                            .executeUpdate("UPDATE `clanmem` SET `n_contribute` = `n_contribute` + 1, `contribute_time` = '"
                                    + Until.toDateString(new Date()) + "', `contribute_text` = '" + Until.getStringNumber(soluong) + " xu"
                                    + "' WHERE `user` = " + us.getIDDB() + ";");
                } else if (type == 1) {
                    ...
                    SQLManager.getStatement()
                            .executeUpdate("UPDATE `clanmem` SET `n_contribute` = `n_contribute` + 1, `contribute_time` = '"
                                    + Until.toDateString(new Date()) + "', `contribute_text` = '" + Until.getStringNumber(soluong) + " lượng"
                                    + "' WHERE `user` = " + us.getIDDB() + ";");
                }
                ms = new Message(45);
                DataOutputStream ds = ms.writer();
